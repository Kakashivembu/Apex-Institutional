...
REM Kill orphaned processes by command line
...
echo   %G%[OK]%X% Ports 8000, 8081, and 5173 cleared (tree-kill)
echo.
...
echo   %DIM%  +-- Apex-Frontend    (Vite direct, port 5173)%X%
echo   %DIM%  +-- Apex-Backend     (Uvicorn, port 8000)%X%
echo   %DIM%  +-- Apex-Forge-Proxy (Forge Guardrails, port 8081)%X%
echo   %DIM%  +-- Apex-CBT-Analyzer%X%
echo.
call pm2 start "%~dp0ecosystem.config.js" --update-env
if !errorlevel! neq 0 (
...
echo %W%  Forge Proxy:%X% %C%http://localhost:8081%X%
