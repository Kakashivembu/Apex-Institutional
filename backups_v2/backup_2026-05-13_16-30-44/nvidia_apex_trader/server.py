    ...
    while True:
        try:
            ...
            for trail_key, state in list(step_trail_state.items()):
                ...
                # Compressed trap prompt — ~150 tokens total
                if side == "long":
                    direction_rule = f"trigger>{current_price:.{digits}f} and <{tp_price:.{digits}f}. SL>{current_sl:.{digits}f} and <{entry_price:.{digits}f}."
                else:
                    direction_rule = f"trigger<{current_price:.{digits}f} and >{tp_price:.{digits}f}. SL<{current_sl:.{digits}f} and >{entry_price:.{digits}f}."
                
                system_prompt = (
                    'Forex risk manager. Return ONLY JSON: '
                    '{"predicted_trigger_price":float,"protective_sl_price":float,"reasoning":"brief"}'
                )
                prompt = (
