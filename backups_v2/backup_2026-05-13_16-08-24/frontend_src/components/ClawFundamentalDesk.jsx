      ...
      <div className="glass-card p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white tracking-tight">Liquidation &amp; Squeeze Radar</h2>
              <p className="text-sm text-slate-500 mt-1">Real-time Binance Futures leverage intelligence</p>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <button
              ...
              className="px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-cyan-500/30 transition-all flex items-center space-x-2 disabled:opacity-40"
            >
              <RefreshCw className={`w-4 h-4 text-slate-400 ${refreshing ? 'animate-spin' : ''}`} />
              <span className="text-sm text-slate-400">{refreshing ? 'Fetching...' : 'Refresh'}</span>
            </button>
        ...
        {loading && !clawData ? (
          <div className="flex flex-col items-center justify-center py-16 space-y-3">
            ...
            <span className="text-slate-400 text-sm">Fetching liquidity data from Binance Futures...</span>
          </div>
        ) : error && !clawData ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              {/* Sentiment Breakdown */}
              <div className="bg-black/40/60 border border-white/5 rounded-2xl p-5">
              ...
              {/* Squeeze Risk */}
              <div className={`bg-black/40/60 border rounded-2xl p-5 ${sqCfg.border} ${sqCfg.pulse ? 'animate-pulse' : ''}`}
                style={sqCfg.pulse ? { boxShadow: `0 0 20px rgba(239,68,68,0.08)` } : {}}>
                <div className="flex items-center space-x-2 mb-4">
                  <AlertTriangle className={`w-4 h-4 ${sqCfg.text}`} />
                  <span className="text-sm font-semibold text-white">Squeeze Risk</span>
                </div>
                <div className="flex items-center justify-center flex-col">
                  <span className={`text-4xl font-black ${sqCfg.text} tracking-wider`}>{squeezeRisk}</span>
                  <span className="text-xs text-slate-500 mt-2">
                    {squeezeRisk === 'HIGH' ? 'Over-leveraged market — potential flush incoming' :
                     squeezeRisk === 'MEDIUM' ? 'Moderate positioning — watch for momentum shift' :
                     'Clean positioning — low liquidation risk'}
                  </span>
                </div>
              </div>

              {/* Dominant Side */}
              <div className={`bg-black/40/60 border rounded-2xl p-5 ${sdCfg.color === 'emerald' ? 'border-emerald-500/20' : sdCfg.color === 'rose' ? 'border-rose-500/20' : 'border-cyan-500/20'}`}>
                <div className="flex items-center space-x-2 mb-4">
                ...
                <div className="flex items-center justify-center flex-col">
                  ...
                  <span className="text-xs text-slate-500 mt-2">
                    {dominantSide === 'LONG' ? 'Majority of positions are long — risk of short squeeze' :
                     dominantSide === 'SHORT' ? 'Majority of positions are short — risk of long squeeze' :
            ...
            <div className="relative group">
              ...
              <div className="relative bg-[#0a0f14]/90 backdrop-blur-xl border border-cyan-500/20 rounded-2xl p-6">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <span className="text-xs text-slate-600 ml-auto">CLAW Agent · Liquidation Hunter v2.1</span>
                </div>
