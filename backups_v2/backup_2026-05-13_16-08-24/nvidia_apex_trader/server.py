...
# =============================================================================
TARGET_SYMBOLS = ["GOLD.i#", "EURUSD#", "GBPUSD#", "USDJPY#", "AUDUSD#", "USDCAD#", "USDCHF#", "NZDUSD#"]
FLEET_SCAN_DELAY = 5
...
async def fetch_real_market_data(symbol: str, skip_consensus: bool = False):
    ...
    try:
      ...
      "status": "strong"
        ...
        # HARD CLAMP: Force AI outputs into Forex scalping range
        raw_sl = float(result.get("stop_loss_pct", 1.5))
        raw_tp = float(result.get("take_profit_pct", 4.0))
        clamped_sl = min(raw_sl, 0.15)   # Max SL: 0.15% (~15 pips)
        clamped_tp = min(raw_tp, 0.30)   # Max TP: 0.30% (~30 pips)
        if raw_sl != clamped_sl or raw_tp != clamped_tp:
            print(f"[CLAMP] AI SL/TP clamped: SL {raw_sl}% → {clamped_sl}% | TP {raw_tp}% → {clamped_tp}%")
        ...
        if trading_enabled and action in ("LONG", "SHORT") and consensus_strength >= 55:
            ...
            active_keys = key_manager.get_active_keys()
            if active_keys:
                # AI risk parameters — CLAMPED to Forex scalping limits
                stop_loss_pct = clamped_sl
                take_profit_pct = clamped_tp
                ...
                async def execute_for_account(key_data):
                    ...
                    try:
                        ...
                        # Safety Check: For micro accounts (<= $1000), use minimum lot to enable trading.
    ...
    while True:
        try:
            ...
            for trail_key, state in list(step_trail_state.items()):
                ...
                prompt = (
                    f"Active Position: {side.upper()} {symbol} from {entry_price:.{digits}f}. "
                    f"Target TP: {tp_price:.{digits}f}. "
                    f"Peak price reached: {peak_price:.{digits}f}. "
                    f"Current price: {current_price:.{digits}f}. "
                    f"Current Stop Loss: {current_sl:.{digits}f}. "
                    ...
                    f"Predict the next probable price target in the current trend direction, "
                    f"and the optimal protective Stop Loss to lock in maximum profit "
                    f"if that target is reached before reversing. "
                )
                if side == "long":
                    prompt += (
                        f"DIRECTION RULES FOR LONG POSITION:\n"
                        f"1. predicted_trigger_price MUST be ABOVE current price ({current_price:.{digits}f}) and BELOW TP ({tp_price:.{digits}f}).\n"
                        f"2. protective_sl_price MUST be BELOW entry ({entry_price:.{digits}f}) and ABOVE current SL ({current_sl:.{digits}f}).\n"
                        f"   Valid SL range: {current_sl:.{digits}f} < protective_sl < {entry_price:.{digits}f}\n"
                        f"   Example: if current SL is {current_sl:.{digits}f}, a better SL might be {current_sl + (entry_price - current_sl) * 0.5:.{digits}f}\n"
                        f"3. NEVER set protective_sl ABOVE the current price. That is a SHORT stop loss, not a LONG one.\n"
                    )
                else:
                    prompt += (
                        f"DIRECTION RULES FOR SHORT POSITION:\n"
                        f"1. predicted_trigger_price MUST be BELOW current price ({current_price:.{digits}f}) and ABOVE TP ({tp_price:.{digits}f}).\n"
                        f"2. protective_sl_price MUST be ABOVE entry ({entry_price:.{digits}f}) and BELOW current SL ({current_sl:.{digits}f}).\n"
                        f"   Valid SL range: {entry_price:.{digits}f} < protective_sl < {current_sl:.{digits}f}\n"
                        f"   Example: if current SL is {current_sl:.{digits}f}, a better SL might be {current_sl - (current_sl - entry_price) * 0.5:.{digits}f}\n"
                        f"3. NEVER set protective_sl BELOW the current price. That is a LONG stop loss, not a SHORT one.\n"
                    )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # GUARDRAIL 2: Auto-correct SL on wrong side of entry (AI confusion)
                        # For LONG: SL must be BELOW entry. For SHORT: SL must be ABOVE entry.
                        sl_wrong_side = False
                        if side == "long" and protective_sl >= entry_price:
                            sl_wrong_side = True
                            # Compute a sensible SL: midpoint between current_sl and entry
                            corrected_sl = current_sl + (entry_price - current_sl) * 0.6
                            print(f"  [AI-TRAP] AUTO-CORRECTED: LONG SL {protective_sl:.{digits}f} was ABOVE entry {entry_price:.{digits}f}")
                            print(f"  [AI-TRAP] Corrected SL: {protective_sl:.{digits}f} -> {corrected_sl:.{digits}f} (60% between current SL and entry)")
                            protective_sl = corrected_sl
                        elif side == "short" and protective_sl <= entry_price:
                            sl_wrong_side = True
                            corrected_sl = current_sl - (current_sl - entry_price) * 0.6
                            print(f"  [AI-TRAP] AUTO-CORRECTED: SHORT SL {protective_sl:.{digits}f} was BELOW entry {entry_price:.{digits}f}")
                            print(f"  [AI-TRAP] Corrected SL: {protective_sl:.{digits}f} -> {corrected_sl:.{digits}f} (60% between current SL and entry)")
                            protective_sl = corrected_sl
