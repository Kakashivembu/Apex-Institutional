...
def check_killzones(timestamp_str: str = "", broker_offset: int = 0) -> bool:
    ...
    The broker_offset parameter allows adjusting for the broker's specific server time.
    For XMGlobal MT5 (typically UTC+3), you might pass an offset to align properly, 
    but by default we assume the incoming time matches the specified windows or offset=0.
    """
    ...
    if not timestamp_str:
        ...
        try:
            # Typical format from backtester: "YYYY-MM-DD HH:MM"
            dt = datetime.strptime(timestamp_str, "%Y-%m-%d %H:%M")
            hour = dt.hour
        except ValueError:
            # Fallback if format is different
            try:
                # E.g. ISO format or similar fallback
                dt = datetime.fromisoformat(timestamp_str)
                hour = dt.hour
            except ValueError:
                # If we absolutely can't parse it, default to True to not block unnecessarily
                return True

    # Adjust hour by broker offset (e.g., if broker is UTC+3 and we want UTC 7-10, 
    # we would define the windows relative to the server time, or adjust the server time back to UTC)
    # Here we adjust the checked hour by the offset. If broker_offset=0, it checks exactly 7-10/13-16.
    adjusted_hour = (hour + broker_offset) % 24
    
    # Widened killzones for small-account maximum frequency
    LONDON_OPEN = 6
    LONDON_CLOSE = 11
    NY_OPEN = 12
    NY_CLOSE = 17
    ...
    in_asian = ASIAN_OPEN <= adjusted_hour <= ASIAN_CLOSE
