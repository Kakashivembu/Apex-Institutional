...
from core.risk_manager import check_circuit_breaker, get_circuit_status, reset_circuit_breaker, midnight_reset_loop
...
# =============================================================================
TARGET_SYMBOLS = ["GOLD.i#", "EURUSD#", "GBPUSD#", "USDJPY#"]
FLEET_SCAN_DELAY = 5
...
# ============================================================
# Per-symbol AI-predicted trigger+SL pairs: { "GOLD.i#": { predicted_trigger_price, protective_sl_price, ... } }
ai_predictive_traps = {}

# Global state for dashboard matrix
macro_matrix_state = {}

# Tier thresholds (percentage from entry) — MAXIMUM PROFIT CAPTURE CONFIG
# 3-Tier MT5 Step-Trailer: fast breakeven → profit step → runner trail
STEP_TRAIL_CONFIG = {
    "initial_sl_pct": 1.2,
    "tier1_trigger_pct": 0.3,       # Lock breakeven early at 0.3% move
    "tier1_sl_pct": 0.05,           # Breakeven + 0.05% profit (micro-profit lock)
    "tier2_trigger_pct": 0.8,       # Start trailing at 0.8% move
    "tier2_trail_pct": 0.3,         # Trail 0.3% behind peak
    "tier2_sl_pct": 0.5,            # Recovery inference: SL ~0.5% profit locked
    "tier3_trigger_pct": 2.0,       # Runner trail at 2.0% move — max capture mode
    "tier3_trail_pct": 0.15,        # Ultra-tight: 0.15% behind peak for runners
    "tier3_sl_pct": 1.5,            # Recovery inference: SL ~1.5% profit locked
    "tier2_min_step_pct": 0.05,     # Fine-grained SL ratcheting: 0.05% steps
}

def calculate_consensus():
    global last_swarm_decisions
    buy_count = sum(1 for d in last_swarm_decisions if d.get("decision") == "BUY")
    sell_count = sum(1 for d in last_swarm_decisions if d.get("decision") == "SELL")
    total = len(last_swarm_decisions)
    
    if total == 0:
        return {"direction": "HOLD", "strength": 50}
    
    if buy_count > sell_count:
        return {"direction": "BUY", "strength": round((buy_count / total) * 100)}
    elif sell_count > buy_count:
        return {"direction": "SELL", "strength": round((sell_count / total) * 100)}
    else:
        return {"direction": "HOLD", "strength": 50}
    ...
    try:
      ...
      "status": "strong"
        ...
        last_consensus = {
            ...
            "stop_loss_pct": result.get("stop_loss_pct", 1.5),
            "take_profit_pct": result.get("take_profit_pct", 4.0),
        ...
        if trading_enabled and action in ("LONG", "SHORT") and consensus_strength >= 55:
            print(f"\n{'='*60}")
            print(f"TRADE EXECUTION TRIGGERED: {action} (Consensus: {consensus_strength}%)")
            print(f"{'='*60}")
            
            active_keys = key_manager.get_active_keys()
            if active_keys:
                # AI risk parameters (same for all accounts)
                stop_loss_pct = result.get("stop_loss_pct", 1.5)
                take_profit_pct = result.get("take_profit_pct", 4.0)
                ...
                async def execute_for_account(key_data):
                    ...
                    try:
                        ...
                        # Dynamic Margin Allocation (Adaptive for account size)
                        # Small accounts need higher risk-per-trade to overcome fixed costs
                        trend_score = result.get("_debug", {}).get("trend_bias", {}).get("score", 0)
                        abs_score = abs(trend_score)

                        # Adaptive base risk: micro accounts use higher % to grow fast
                        if acct_equity <= 100:
                        ...
                        # Split margin if there are concurrent positions
                        ...
                        print(f"[TRADE:{t_account}] Dynamic Margin applied: Trend Score {abs_score} -> Base Risk {base_risk*100:.0f}%, Split for {active_count+1} positions -> {risk_pct*100:.2f}% (${max_risk_usd:.2f})")
                        ...
                        # Inverse Position Sizing Calculation
                        # 1% move risk calculation:
                        sl_distance_price = mark_price * (stop_loss_pct / 100.0)
                        # Dollar risk per 1 standard lot = sl_distance_price * trade_contract_size (e.g., 100 oz for Gold)
                        ...
                        # Safety Check: For micro accounts (<= $200), use minimum lot to enable trading.
                        # The step-trail system + tight SL protects capital. For larger accounts, enforce strict risk limit.
                        if calculated_lot < min_lot:
                            if acct_equity <= 200:
                                print(f"[TRADE:{t_account}] Micro-account override: Using min lot {min_lot} (calculated {calculated_lot:.4f}). Step-trail will protect capital.")
                                calculated_lot = min_lot
                            else:
                                print(f"[TRADE:{t_account}] SL too wide for account size, rejecting to protect capital. (Calculated lot: {calculated_lot:.4f} < Min: {min_lot})")
                                return
                            
                        # Snap lot size to broker volume step
                        ...
                        # Persist trade to SQLite for lifetime Bot Performance tracking
                        if order_result.get("success"):
                            entry_fee = contract_size * mark_price * 0.0001  # MT5 spread estimate ~1 pip
                            ai_reasoning = result.get("reasoning", "")
                            # Run DB write in thread pool so we don't block the consensus loop
        ...
        elif action in ("LONG", "SHORT") and not trading_enabled:
...
async def step_trailing_loop():
    ...
    Rate-limit safety:
    - Tier 1: API call sent once when hurdle is crossed
    - Tier 2: API call only if new SL improves by tier2_min_step_pct
    - 0.2s delay between accounts
    """
    ...
    while True:
        try:
                    ...
                    for pos in open_positions:
                        ...
                        if trail_key not in step_trail_state:
                            ...
                            if move_pct_from_entry >= cfg.get("tier3_trigger_pct", 999):
                                recovered_tier = 3
                                if side == "long":
                                    computed_sl = round(round(mark_price * (1 - cfg.get("tier3_trail_pct", 0.15) / 100) / tick) * tick, 5)
                                else:
                                    computed_sl = round(round(mark_price * (1 + cfg.get("tier3_trail_pct", 0.15) / 100) / tick) * tick, 5)
                            elif move_pct_from_entry >= cfg.get("tier2_trigger_pct", 999):
                                recovered_tier = 2
                                if side == "long":
                                    computed_sl = round(round(mark_price * (1 - cfg.get("tier2_trail_pct", 0.3) / 100) / tick) * tick, 5)
                                else:
                                    computed_sl = round(round(mark_price * (1 + cfg.get("tier2_trail_pct", 0.3) / 100) / tick) * tick, 5)
                            elif move_pct_from_entry >= cfg["tier1_trigger_pct"]:
                                recovered_tier = 1
                                if side == "long":
                                    computed_sl = round(round(entry_price * (1 + cfg["tier1_sl_pct"] / 100) / tick) * tick, 5)
                                else:
                                    computed_sl = round(round(entry_price * (1 - cfg["tier1_sl_pct"] / 100) / tick) * tick, 5)
                            else:
                                recovered_tier = 0
                                if side == "long":
                                    computed_sl = round(entry_price * (1 - cfg["initial_sl_pct"] / 100), 5)
                                else:
                                    computed_sl = round(entry_price * (1 + cfg["initial_sl_pct"] / 100), 5)
                            ...
                            if is_recovery:
                                ...
                                print(f"  Account: {account_name} | Entry: ${entry_price:.1f} | Current: ${mark_price:.1f} (+{move_pct_from_entry:.1f}%)")
                                print(f"  Exchange SL: ${existing_exchange_sl:.1f} | Computed SL: ${computed_sl:.1f} | Using: ${initial_sl:.1f}")
                                print(f"  Resuming at TIER {recovered_tier} ({tier_names.get(recovered_tier, '?')}) | Order ID: {existing_sl_order_id}")
                                print(f"{'='*60}\n")
                            else:
                                print(f"[STEP-TRAIL:{account_name}] NEW Tracking {side.upper()} {symbol} | Entry: ${entry_price:.1f} | Initial SL: ${initial_sl:.1f}")
                        ...
                        tick = 0.1  # Default MT5 tick size for Forex/Metals
                        ...
                        if move_pct >= cfg.get("tier3_trigger_pct", 999):
                            ...
                            if side == "long":
                                ...
                                raw_sl = peak * (1 + cfg.get("tier3_trail_pct", 0.15) / 100)
                            candidate_sl = round(round(raw_sl / tick) * tick, 5)
                        ...
                        # TIER 1: Breakeven lock
                            ...
                            math_tier = 1
                            if side == "long":
                                math_sl = round(round(entry_price * (1 + cfg["tier1_sl_pct"] / 100) / tick) * tick, 5)
                            else:
                                math_sl = round(round(entry_price * (1 - cfg["tier1_sl_pct"] / 100) / tick) * tick, 5)
                            math_wants_update = True
                        ...
                        trap = ai_predictive_traps.get(symbol)
                        ...
                        if trap and not trap.get("executed"):
                            ...
                            if trap_trigger > 0 and trap_protective > 0:
                                ...
                                if trap_hit and sl_improves:
                                    trap_sl = round(round(trap_protective / tick) * tick, 1)
                                    trap_triggered = True
                        ...
                        if sl_source and final_sl != state["current_active_sl"]:
                            if side == "long" and final_sl > state["current_active_sl"]:
                                ...
                                print(f"  [GUARD] BLOCKED {sl_source} SL for {side.upper()} {symbol}: ${state['current_active_sl']:.1f} -> ${final_sl:.1f} (wrong direction!)")
                        ...
                        if sl_direction_valid:
                            if sl_source == "ai_trap":
                                ...
                                print(f"  {side.upper()} {symbol} | Price ${mark_price:.1f} hit trigger ${trap.get('predicted_trigger_price', 0):.1f}")
                                print(f"  SL: ${state['current_active_sl']:.1f} -> ${final_sl:.1f} (AI Trap)")
                                if math_wants_update:
                                    print(f"  Math tier would have set: ${math_sl:.1f} (T{math_tier}) — AI was tighter")
                                print(f"  AI Reasoning: {trap.get('reasoning', 'N/A')[:120]}")
                                print(f"{'='*60}")
                            else:
                                print(f"\n[UNIFIED-TRAIL:{account_name}] >> {tier_names.get(final_tier, '?')} TRIGGERED (Math)")
                                print(f"  {side.upper()} {symbol} | Entry: ${entry_price:.1f} | Peak: ${peak:.1f} (+{move_pct:.1f}%)")
                                print(f"  SL: ${state['current_active_sl']:.1f} -> ${final_sl:.1f}")
                                if trap and not trap.get("executed"):
                                    print(f"  AI Trap waiting at ${trap.get('predicted_trigger_price', 0):.1f} (not yet triggered)")
                            ...
                            if state.get("sl_order_id"):
                                ...
                                if mt5_result is None:
                                    ...
                                    print(f"  [MT5 REJECTION] SL modification order_send() failed. MT5 Error Code: {error}")
                                    result = {"success": False}
                                elif mt5_result.retcode != mt5.TRADE_RETCODE_DONE:
                                    print(f"  [MT5 REJECTION] SL modification rejected by broker. Retcode: {mt5_result.retcode} | Comment: {mt5_result.comment}")
                                    result = {"success": False}
                                else:
                            ...
                            if result.get("success"):
                                ...
                                print(f"  [OK] SL LOCKED at ${final_sl:.1f} via {sl_source.upper()}")
            ...
            # 0.2s delay between accounts to respect rate limits
            await asyncio.sleep(0.2)
    ...
    while True:
        try:
            ...
            for trail_key, state in list(step_trail_state.items()):
                ...
                # ── DOM X-RAY: Fetch L2 Orderbook for AI Trap positioning ──
                try:
                    dom_data = await fetch_dom_imbalance(symbol)
                ...
                print(f"  Entry: ${entry_price:.1f} | Peak: ${peak_price:.1f} | Current: ${current_price:.1f} | SL: ${current_sl:.1f}")
                
                prompt = (
                    f"Active Position: {side.upper()} from ${entry_price:.2f}. "
                    f"Target TP: ${tp_price:.2f}. "
                    f"Peak price reached: ${peak_price:.2f}. "
                    f"Current price: ${current_price:.2f}. "
                    f"Current Stop Loss: ${current_sl:.2f}. "
                    f"Current resting Order Book data: {dom_data}. "
                    f"CRITICAL: Use these physical buy/sell walls to position your 'predicted_trigger_price'. "
                    f"Set traps just ahead of massive walls to safely front-run reversals. "
                    f"Predict the next probable price target in the current trend, "
                    f"and the optimal protective Stop Loss to lock in maximum profit "
                    f"if that target is reached before reversing. "
                    f"The protective SL MUST be more favorable than the current SL (${current_sl:.2f}). "
                    f"For {side.upper()}: {'higher SL = more profit locked' if side == 'long' else 'lower SL = more profit locked'}. "
                    f"CRITICAL RULE: Your predicted_trigger_price MUST mathematically fall BETWEEN the current_price (${current_price:.2f}) and the Target TP (${tp_price:.2f}). "
                    f"It is physically impossible to trigger a trap past the TP because the position closes at TP. "
                    f"{'For SHORT: trigger MUST be GREATER than TP and LESS than current price.' if side == 'short' else 'For LONG: trigger MUST be LESS than TP and GREATER than current price.'} "
                    f"Set the trap just before the TP to catch front-running reversals."
                )
                
                system_prompt = (
                    "You are an elite quantitative risk manager for Forex and Precious Metals. "
                    "Given an active position, predict where price is likely heading next "
                    "and set a protective stop loss that locks in maximum profit. "
                    "Return ONLY valid JSON: "
                    '{"predicted_trigger_price": float, "protective_sl_price": float, "reasoning": "brief explanation"}'
                )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Robust JSON parsing
                        try:
                            trap_data = json.loads(clean)
                        except json.JSONDecodeError:
                            import re
                            match = re.search(r'\{[^{}]+\}', clean)
                            if match:
                                trap_data = json.loads(match.group(0))
                            else:
                                print(f"  [AI-TRAP] Failed to parse JSON: {clean[:200]}")
                                continue
                        ...
                        if tp_price > 0:
                            if side == "short" and trigger <= tp_price:
                                ...
                                print(f"  [AI-TRAP] CLAMPED: Short trigger ${old_trigger:.1f} was below TP ${tp_price:.1f} -> adjusted to ${trigger:.1f} (front-running TP)")
                            elif side == "long" and trigger >= tp_price:
                                old_trigger = trigger
                                trigger = tp_price - (tp_price - current_price) * 0.15  # 15% below TP
                                print(f"  [AI-TRAP] CLAMPED: Long trigger ${old_trigger:.1f} was above TP ${tp_price:.1f} -> adjusted to ${trigger:.1f} (front-running TP)")
                        
                        # GUARDRAIL 2: Validate trap direction
                        # For LONG: trigger should be below current (dip prediction), protective SL between entry and current SL
                        # For SHORT: trigger should be above current (bounce prediction), protective SL between entry and current SL
                        valid_trap = False
                        if side == "long":
                            # Trigger must be below current price, and protective SL must be above entry (to lock profit)
                            valid_trap = trigger < current_price and protective_sl > entry_price
                        elif side == "short":
                            # Trigger must be above current price, and protective SL must be below entry (to lock profit)
                            valid_trap = trigger > current_price and protective_sl < entry_price
                        
                        if not valid_trap:
                            print(f" [AI-TRAP] REJECTED: Trap direction invalid for {side.upper()}")
                            print(f" Trigger: ${trigger:.1f} (current: ${current_price:.1f})")
                            print(f" Protective SL: ${protective_sl:.1f} (entry: ${entry_price:.1f})")
                            continue
                        
                        # Store the trap
                        ai_predictive_traps[symbol] = {
                        ...
                        print(f"  [AI-TRAP] SET for {symbol}: If price hits ${trigger:.1f}, SL snaps to ${protective_sl:.1f}")
...
async def market_data_loop():
    ...
    while True:
        ...
        try:
            ...
            if current_equity > 0:
                circuit_result = check_circuit_breaker(current_equity)
            ...
            await manager.broadcast({
                ...
                "data": {
                    ...
                    "step_trail": {k: {"tier": v["current_tier"], "sl": v["current_active_sl"], "tp": v.get("current_tp", 0), "ticket": v.get("ticket", v.get("product_id")), "side": v["side"], "entry": v["entry_price"], "peak": v.get("peak_price", v.get("highest_price", v.get("lowest_price", 0))), "ai_target": ai_predictive_traps.get(v.get("symbol", ""), {}).get("predicted_trigger_price", None)} for k, v in step_trail_state.items()},
    ...
    try:
        ...
        for pos in profitable:
            ...
            if result.get("success"):
                ...
                print(f"[CLOSE-PROFITABLE] ✓ Closed {symbol} #{pos_ticket} | PnL: ${pnl:+.2f}")
            else:
    ...
    async def _run_task():
        ...
        try:
            if symbol == "ALL_FLEET":
                ...
                # Dynamic watchlist from MT5 Market Watch is ideal, but hardcoding primary majors for now
                fleet_watchlist = ["GOLD.i#", "EURUSD.i#", "GBPUSD.i#", "USDJPY.i#"]
                result = await run_fleet_backtest(
                    watchlist=fleet_watchlist,
