...
async def evaluate_market(memory_text: str, market_data_text: str, margin: float = 0, dom_data: str = "", active_positions: list = None, force_run: bool = False, active_symbol: str = "GOLD", live_asset_price: float = 0.0) -> dict:
    ...
    # 2/3 majority consensus: any two agents aligned unlock the direction.
    bullish_set = {"BUY", "LONG", "BULLISH"}
    bearish_set = {"SELL", "SHORT", "BEARISH"}
    bullish_votes = sum([
        macro_decision in bullish_set,
        scalper_decision in bullish_set,
        claw_vote in bullish_set
    ])
    bearish_votes = sum([
        macro_decision in bearish_set,
        scalper_decision in bearish_set,
        claw_vote in bearish_set
    ])
    ...
    if bullish_votes >= 2 and bullish_votes > bearish_votes:
        ...
        final_action = "HOLD"
        # Log the exact split so we can see why trades were blocked
        if macro_decision != "HOLD" or scalper_decision != "HOLD" or claw_vote in ("BULLISH", "BEARISH"):
            reasoning = f"No 2/3 majority across agents (Bullish: {bullish_votes}, Bearish: {bearish_votes}). Trade BLOCKED."
            print(f"[CONSENSUS] No 2/3 majority (Macro: {macro_decision}, Scalper: {scalper_decision}, CLAW: {claw_vote}). Bullish={bullish_votes}, Bearish={bearish_votes}. Trade BLOCKED.")
