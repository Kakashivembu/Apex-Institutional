...
const FOREX_SESSIONS = [
  { id: 'sydney', label: 'Sydney Session', emoji: '🦘', open: 22, close: 7, color: 'blue', gradient: 'from-blue-500 to-indigo-600', glow: 'rgba(59,130,246,0.6)', bg: 'blue-500' },
  { id: 'asian', label: 'Asian/Tokyo', emoji: '🌏', open: 23, close: 8, color: 'purple', gradient: 'from-purple-500 to-fuchsia-600', glow: 'rgba(168,85,247,0.6)', bg: 'purple-500' },
  { id: 'london',  label: 'London Session', emoji: '🇬🇧', open: 7, close: 16, color: 'cyan', gradient: 'from-cyan-500 to-blue-600', glow: 'rgba(6,182,212,0.8)', bg: 'cyan-500' },
  { id: 'newyork',   label: 'New York Session', emoji: '🇺🇸', open: 12, close: 21, color: 'amber', gradient: 'from-amber-400 to-orange-500', glow: 'rgba(245,158,11,0.8)', bg: 'amber-500' },
];
...
const normalizeBrokerSymbol = (symbol = '') => {
  ...
  return clean.replace(/\.I#$/i, '').replace(/#$/g, '').replace(/\.I$/i, '').replace(/\.X$/i, '').replace(/\.x$/i, '');
};
...
const TradingCommandCenter = ({ marketData, wsConnected, refreshData, inrRate = 84.5 }) => {
  ...
  const dynamicAssets = useMemo(() => {
    const assets = [...MACRO_ASSETS];
    const existingIds = new Set(assets.map(a => a.id));
    const fleet = localMarketData?.fleet_symbols || [];
    
    fleet.forEach(sym => {
      const normId = normalizeBrokerSymbol(sym);
      if (!existingIds.has(sym) && !assets.find(a => normalizeBrokerSymbol(a.id) === normId)) {
        assets.push({
          id: sym,
          symbol: getTradingViewSymbol(sym),
          label: sym.replace(/\.x$/i, '').replace(/\.i$/i, '').replace(/#$/g, '')
        });
        existingIds.add(sym);
      }
    });
    return assets;
  }, [localMarketData?.fleet_symbols]);
  ...
  useEffect(() => {
    ...
    const mapped = dynamicAssets.find((a) => a.id === radarSymbol || normalizeBrokerSymbol(a.id) === normalizedRadarSymbol);
    if (mapped && mapped.id !== activeChartAsset.id) {
      setActiveChartAsset(mapped);
    }
  }, [localMarketData.active_symbol, activeChartAsset.id, dynamicAssets]);
  ...
  const uniqueActiveSymbols = useMemo(() => {
    // MT5 positions are always active if they exist — no 'status' field
  ...
  const getPositionSL = (pos) => {
    const trail = Object.values(stepTrail).find(t => String(t.ticket) === String(pos.ticket || pos.symbol));
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-3 mb-2">
            <select 
              ...
              onChange={(e) => setActiveChartAsset(dynamicAssets.find(a => a.id === e.target.value))}
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 font-bold uppercase tracking-wider">{positions.length} Active Contracts</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {(() => {
              const profitableCount = positions.filter(p => Number(p.pnl || 0) >= 0).length;
              return (
                ...
                return (
                  <React.Fragment key={index}>
                    <tr className="border-b border-white/[0.05] hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="py-5 text-base text-white font-mono font-bold">{Math.abs(pos.qty || pos.volume || 0)}</td>
                      <td className="py-5 text-base text-white font-mono font-bold">${(pos.entry || pos.entry_price)?.toFixed(5) || '—'}</td>
                      <td className="py-5 text-base text-slate-300 font-mono">
                        <AnimatedPrice value={pos.current || pos.mark_price || assetMarketData.last_price || 0} />
                      ...
                      <td className={`py-5 text-lg font-black font-heading ${netPnl >= 0 ? 'text-emerald-400' : 'text-pink-500'}`}>
                        ...
                        <span className={`block text-[10px] font-medium ${netPnl >= 0 ? 'text-emerald-600' : 'text-pink-600'}`}>
                          ₹{(netPnl * (inrRate || USD_TO_INR)).toLocaleString(undefined, {maximumFractionDigits: 0})}
                        </span>
                      </td>
                      <td className="py-5">
