...
// ─── Main Component ───────────────────────────────────────
const BotPerformance = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      {/* FIX #6: Overtrading Alert */}
      {(s.today_trades || 0) > 20 && (
        <div className="glass-card rounded-[1.5rem] p-4 border-2 border-amber-500/40 bg-amber-500/5">
          <div className="flex items-center gap-3">
            <AlertTriangle className="w-6 h-6 text-amber-400 shrink-0 animate-pulse" />
            <div className="flex-1">
              <p className="text-amber-300 text-sm font-bold">
                ⚠️ Overtrading Alert: {s.today_trades} trades today
              </p>
              <p className="text-amber-400/70 text-xs mt-0.5">
                High trade frequency increases spread costs and reduces edge.{' '}
                {onNavigate ? (
                  <button
                    onClick={() => onNavigate('settings')}
                    className="underline text-amber-300 hover:text-amber-200 font-bold transition-colors"
                  >
                    Enable Trade Cooldown in Settings →
                  </button>
                ) : (
                  <span>Consider enabling Trade Cooldown in Settings.</span>
                )}
              </p>
            </div>
          </div>
